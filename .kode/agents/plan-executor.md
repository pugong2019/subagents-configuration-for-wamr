...
1. **Identify Function Accessibility**:
   ```bash
   # Check if function is static (internal)
   grep -n "^static.*function_name" source_file.c
   
   # If static, find public APIs that call it
   grep -r "function_name" . --include="*.c" --include="*.h"
   ```

2. **Call Chain Analysis(For Example)**:
   - **Static Function**: `set_error_buf_v()` → **Public API**: `aot_load_from_aot_file()`
   - **Static Function**: `exchange_uint16()` → **Public API**: Module loading with endianness
   - **Static Function**: `destroy_import_*()` → **Public API**: `aot_unload()`
...
3. **Test Through Public APIs**:
   ...
   TEST_F(ModuleTest, PublicAPI_CallsStaticFunction_ValidatesInternalBehavior) {
       ...
       // Validate that internal error formatting (set_error_buf_v) worked correctly
...
### Function Accessibility Classification
...
#### **Call Chain Mapping Protocol**:
For each static function in the plan:
...
### Example Call Chain Testing

```cpp
// Target: static function set_error_buf_v() in aot_loader.c
// Public Caller: aot_load_from_aot_file()

TEST_F(AOTLoaderTest, AotLoadFromAotFile_InvalidInput_FormatsErrorCorrectly) {
    // Arrange: Create conditions that will trigger set_error_buf_v()
    const uint8 *invalid_buf = nullptr;
    LoadArgs args = {0};
    
    // Act: Call public API that internally calls set_error_buf_v()
    AOTModule *module = aot_load_from_aot_file(invalid_buf, 0, &args, error_buf, error_buf_size);
    
    // Assert: Validate that set_error_buf_v() formatted the error correctly
    ASSERT_EQ(module, nullptr);
    ASSERT_GT(strlen(error_buf), 0);
    ASSERT_TRUE(strstr(error_buf, "AOT module load failed:") != nullptr);
}
...
**Note:** Not ALL Features need wasm
...
- [ ] Coverage improvement is measurable for the target functions
